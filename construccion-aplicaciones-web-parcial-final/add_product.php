...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    executeStatement($dbConnection, $statement, "./product_inventory.php", 'Error al registrar el producto', $action);
}
                ...
                <form action="add_product.php" method="post">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name"
                               ...
                               value="<?php echo $action === "update" ? $product_data["name"] : "" ?>" required>
                    </div>
                    <div class="form-group mb-3">
                        <label for="price">Precio unitario</label>
                        <input type="number" class="form-control" min="0" id="price" name="price"
                               placeholder="Ingrese el precio precio unitario del producto"
                               value="<?php echo $action === "update" ? $product_data["price"] : "0" ?>" required>
                    </div>
                    <div class="form-group mb-3">
                        <label for="quantity">Cantidad</label>
                        <input type="number" oninput="this.value = Math.round(this.value);" min="0"
                               class="form-control" id="quantity" name="quantity"
                               placeholder="Ingrese la cantidad de productos disponibles"
                               value="<?php echo $action === "update" ? $product_data["quantity"] : "1" ?>" required>
                    </div>
